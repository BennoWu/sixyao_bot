...
def getDrawRiceGua( note = "" , printMode = False ):
	...
	for i in range(4):
		...
		eachRawDrawFlex_Buf = eachRawDrawFlex.replace( "__UP1__" , randList1[0] ).replace( "__MID1__" , randList1[1] ).replace( "__DN1__" , randList1[2] ).replace( "__NUM1__", str(i*4+1)).replace( "__DRAW1__" , ",".join(randList1) + " // " + note ).replace( "#eeeeea" , color1).replace( "__SIX1__" , "s+" + ",".join(randList1)+ " // " + note ).replace( "__SIX_DSP1__" , "s+" + ",".join(randList1)+ " // " + note ).replace( "__NOTE__", "s+" + note )
